import { $ as randomInRange, A as executeOnSingleOrMultiple, B as rectBounce, C as updateColor, D as deepExtend, E as circleBounceDataFromParticle, F as isPointInside, G as clamp, H as OutModeDirection, I as isSsr, J as getDistances, K as degToRad, L as itemFromArray, M as initParticleNumericAnimationValue, N as isDivModeEnabled, O as divMode, P as isInArray, Q as parseAlpha, R as itemFromSingleOrMultiple, S as rgbToHsl, T as circleBounce, U as DestroyType, V as updateAnimation, W as AnimationStatus, X as getRangeMax, Y as getRandom, Z as getRangeValue, _ as getStyleFromRgb, a as InteractorType, at as errorPrefix, b as rangeColorToHsl, c as CollisionMode, ct as mouseMoveEvent, d as drawLine$1, et as setRangeValue, f as colorMix, g as getStyleFromHsl, h as getLinkRandomColor, i as ParticleOutType, it as isObject, j as getLogger, k as divModeExecute, l as DivType, m as getLinkColor, n as Circle, nt as isArray, o as OutMode, ot as millisecondsToSeconds, p as getHslAnimationFromHsl, q as getDistance, r as Rectangle, rt as isNull, s as ValueWithRandom, tt as Vector, u as OptionsColor, v as hslToRgb, w as calculateBounds, x as rangeColorToRgb, y as hslaToRgba, z as loadFont } from "./browser-DRi8p1LK.js";
...
var half$7 = .5, double$8 = 2, minVelocity$4 = 0, identity$4 = 1, moveSpeedFactor = 60, minSpinRadius = 0, spinFactor = .01, doublePI$2 = Math.PI * double$8;
function applyDistance(particle) {
	...
	if ((((hDistance && dxFixed >= hDistance) ?? false) || ((vDistance && dyFixed >= vDistance) ?? false)) && !particle.misplaced) {
		...
		if (hDistance) particle.velocity.x = particle.velocity.y * half$7 - particle.velocity.x;
		if (vDistance) particle.velocity.y = particle.velocity.x * half$7 - particle.velocity.y;
...
function move(particle, moveOptions, moveSpeed, maxSpeed, moveDrift, reduceFactor, delta) {
	...
	const gravityOptions = particle.gravity, gravityFactor = gravityOptions?.enable && gravityOptions.inverse ? -1 : identity$4;
...
function applyPath(particle, delta) {
	...
	if (pathOptions.clamp) {
		particle.velocity.x = clamp(particle.velocity.x, -1, identity$4);
		particle.velocity.y = clamp(particle.velocity.y, -1, identity$4);
	}
...
var doublePI$1 = Math.PI * 2, minAngle$1 = 0, origin$4 = {
	x: 0,
	y: 0
};
...
var minFactor = 1, identity$3 = 1, minRadius$3 = 0;
function processAttract(engine, container, position, attractRadius, area, queryCb) {
	...
	for (const particle of query) {
		const { dx, dy, distance } = getDistances(particle.position, position), velocity = attractOptions.speed * attractOptions.factor, attractFactor = clamp(engine.getEasing(attractOptions.easing)(identity$3 - distance / attractRadius) * velocity, minFactor, attractOptions.maxSpeed), normVec = Vector.create(!distance ? velocity : dx / distance * attractFactor, !distance ? velocity : dy / distance * attractFactor);
...
var squareExp = 2, half$6 = .5, halfPI = Math.PI * half$6, double$7 = 2, toleranceFactor = 10, minRadius$2 = 0;
...
function singleSelectorBounce(container, selector, div, bounceCb) {
	...
	query.forEach((item) => {
		...
		}, radius = elem.offsetWidth * half$6 * pxRatio, tolerance = toleranceFactor * pxRatio;
		bounceCb(pos, radius, div.type === DivType.circle ? new Circle(pos.x, pos.y, radius + tolerance) : new Rectangle(elem.offsetLeft * pxRatio - tolerance, elem.offsetTop * pxRatio - tolerance, elem.offsetWidth * pxRatio + tolerance * double$7, elem.offsetHeight * pxRatio + tolerance * double$7));
	});
}
...
var bubbleMode = "bubble", minDistance$5 = 0, defaultClickTime = 0, double$6 = 2, defaultOpacity$2 = 1, ratioOffset = 1, defaultBubbleValue = 0, minRatio = 0, half$5 = .5, defaultRatio$1 = 1;
	...
	constructor(container, engine) {
		...
		this._singleSelectorHover = (delta, selector, div) => {
			...
			selectors.forEach((item) => {
				...
				}, repulseRadius = elem.offsetWidth * half$5 * pxRatio, area = div.type === DivType.circle ? new Circle(pos.x, pos.y, repulseRadius) : new Rectangle(elem.offsetLeft * pxRatio, elem.offsetTop * pxRatio, elem.offsetWidth * pxRatio, elem.offsetHeight * pxRatio), query = container.particles.quadTree.query(area, (p) => this.isEnabled(p));
				for (const particle of query) {
...
var repulseMode = "repulse", minDistance$2 = 0, repulseRadiusFactor = 6, repulseRadiusPower = 3, squarePower$1 = 2, minRadius$1 = 0, minSpeed = 0, easingOffset = 1, half$4 = .5;
...
var origin$3 = {
	x: 0,
	y: 0
}, defaultFrame = 0, half$3 = .5, initialTime = 0, firstIndex = 0, defaultLoopCount = 0;
...
function drawGif(data) {
	...
	const pos = {
		x: -image.gifData.width * half$3,
		y: -image.gifData.height * half$3
...
var double$5 = 2, defaultAlpha = 1, sides$3 = 12, defaultRatio = 1;
...
var noTime = 0, identity$2 = 1, infiniteValue = -1;
...
var attractFactor = 1e3, identity$1 = 1;
var Attractor = class extends ParticlesInteractorBase {
	...
	interact(p1) {
		...
		for (const p2 of query) {
			...
			const { dx, dy } = getDistances(pos1, p2.getPosition()), rotate = p1.options.move.attract.rotate, ax = dx / (rotate.x * attractFactor), ay = dy / (rotate.y * attractFactor), p1Factor = p2.size.value / p1.size.value, p2Factor = identity$1 / p1Factor;
...
var half$1 = .5, absorbFactor = 10, minAbsorbFactor = 0;
...
var minOpacity = 0, minWidth = 0, minDistance = 0, half = .5, maxFrequency = 1;
...
var yFactor = 1.66, sides$1 = 3, double$2 = 2;
...
var double$1 = 2, doublePI = Math.PI * double$1, identity = 1, doublePIDeg = 360;
var RotateUpdater = class {
	...
	init(particle) {
		...
		if (rotateDirection === RotateDirection.random) rotateDirection = Math.floor(getRandom() * double$1) > 0 ? RotateDirection.counterClockwise : RotateDirection.clockwise;
...
var fixFactor = Math.sqrt(2), double = 2;
