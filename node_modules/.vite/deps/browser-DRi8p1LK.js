...
var Vector3d = class Vector3d {
	constructor(xOrCoords, y, z) {
		...
		} else throw new Error(`${errorPrefix} Vector3d not initialized correctly`);
	}
	static get origin() {
...
//#endregion
//#region node_modules/@tsparticles/engine/browser/Utils/NumberUtils.js
...
function deepExtend(destination, ...sources) {
	for (const source of sources) {
		...
		for (const key in source) {
			...
			const value = source[key], destDict = destination;
...
function updateAnimation(particle, data, changeDirection, destroyType, delta) {
	...
	switch (data.status) {
		case AnimationStatus.increasing:
			...
			break;
		case AnimationStatus.decreasing: if (data.value <= minValue) {
...
function cloneStyle(style) {
	...
	if (!style) return clonedStyle;
	for (const key in style) {
		const styleKey = style[key];
		if (!Object.prototype.hasOwnProperty.call(style, key) || isNull(styleKey)) continue;
		const styleValue = style.getPropertyValue?.(styleKey);
		if (!styleValue) continue;
		const stylePriority = style.getPropertyPriority?.(styleKey);
		if (!stylePriority) clonedStyle.setProperty?.(styleKey, styleValue);
		else clonedStyle.setProperty?.(styleKey, styleValue, stylePriority);
	}
	return clonedStyle;
}
...
function stringToRgba(engine, input) {
	if (!input) return;
	for (const manager of engine.colorManagers.values()) if (input.startsWith(manager.stringPrefix)) return manager.parseString(input);
}
function rangeColorToRgb(engine, input, index, useIndex = true) {
	if (!input) return;
	const color = isString(input) ? { value: input } : input;
	if (isString(color.value)) return colorToRgb(engine, color.value, index, useIndex);
	if (isArray(color.value)) return rangeColorToRgb(engine, { value: itemFromArray(color.value, index, useIndex) });
...
function colorToRgb(engine, input, index, useIndex = true) {
	if (!input) return;
	const color = isString(input) ? { value: input } : input;
	if (isString(color.value)) return color.value === "random" ? getRandomRgbColor() : stringToRgb(engine, color.value);
	if (isArray(color.value)) return colorToRgb(engine, { value: itemFromArray(color.value, index, useIndex) });
	for (const manager of engine.colorManagers.values()) {
...
function getRandomRgbColor(min) {
	const fixedMin = min ?? 0, fixedMax = 256;
	return {
...
function colorMix(color1, color2, size1, size2) {
	...
	if (rgb1.r === void 0) rgb1 = hslToRgb(color1);
	if (rgb2.r === void 0) rgb2 = hslToRgb(color2);
	return {
...
function getLinkRandomColor(engine, optColor, blink, consent) {
	...
	if (color === "random") {
		if (consent) return rangeColorToRgb(engine, { value: color });
		if (blink) return randomColorValue;
		return "mid";
	} else if (color === "mid") return "mid";
	else return rangeColorToRgb(engine, { value: color });
}
...
function drawParticle(data) {
	const { container, context, particle, delta, colorStyles, backgroundMask, composite, radius, opacity, shadow, transform } = data, pos = particle.getPosition(), angle = particle.rotation + (particle.pathRotation ? particle.velocity.angle : 0), rotateData = {
		sin: Math.sin(angle),
		cos: Math.cos(angle)
	}, rotating = !!angle, transformData = {
		a: rotateData.cos * (transform.a ?? defaultTransform.a),
		b: rotating ? rotateData.sin * (transform.b ?? 1) : transform.b ?? defaultTransform.b,
		c: rotating ? -rotateData.sin * (transform.c ?? 1) : transform.c ?? defaultTransform.c,
...
function setStyle(canvas, style, important = false) {
	...
	for (const key in elementStyle) {
		if (!Object.prototype.hasOwnProperty.call(elementStyle, key)) continue;
	...
	for (const key of keys) {
		...
		else elementStyle.setProperty(key, value, important ? "important" : "");
	}
}
var Canvas = class {
	constructor(container, engine) {
		...
		this._getPluginParticleColors = (particle) => {
			...
			for (const plugin of this._colorPlugins) {
				if (!fColor && plugin.particleFillColor) fColor = rangeColorToHsl(this._engine, plugin.particleFillColor(particle));
				if (!sColor && plugin.particleStrokeColor) sColor = rangeColorToHsl(this._engine, plugin.particleStrokeColor(particle));
				if (fColor && sColor) break;
			}
			return [fColor, sColor];
		};
		this._initCover = async () => {
			const cover = this.container.actualOptions.backgroundMask.cover, color = cover.color;
			if (color) {
		...
		this._initStyle = () => {
			...
			if (!element) return;
			if (this._fullScreen) this._setFullScreenStyle();
			else this._resetOriginalStyle();
			for (const key in options.style) {
				if (!key || !options.style || !Object.prototype.hasOwnProperty.call(options.style, key)) continue;
		...
		this._initTrail = async () => {
			const trail = this.container.actualOptions.particles.move.trail, trailFill = trail.fill;
			if (!trail.enable) return;
			const opacity = 1 / trail.length;
			if (trailFill.color) {
	...
	clear() {
		...
		if (options.backgroundMask.enable) this.paint();
			...
			if (trailFill.color) this._paintBase(getStyleFromRgb(trailFill.color, trailFill.opacity));
	...
	drawParticle(particle, delta) {
		...
		this.draw((ctx) => {
			const container = this.container, options = container.actualOptions, zIndexOptions = particle.options.zIndex, zIndexFactor = 1 - particle.zIndexFactor, zOpacityFactor = zIndexFactor ** zIndexOptions.opacityRate, opacity = particle.bubble.opacity ?? particle.opacity?.value ?? 1, strokeOpacity = particle.strokeOpacity ?? opacity, zOpacity = opacity * zOpacityFactor, zStrokeOpacity = strokeOpacity * zOpacityFactor, transform = {}, colorStyles = { fill: fColor ? getStyleFromHsl(fColor, zOpacity) : void 0 };
	...
	initBackground() {
		const background = this.container.actualOptions.background, element = this.element;
		if (!element) return;
		const elementStyle = element.style;
		if (!elementStyle) return;
		if (background.color) {
	...
	loadCanvas(canvas) {
		...
		this._generated = canvas.dataset && "generated" in canvas.dataset ? canvas.dataset[generatedAttribute] === "true" : this._generated;
		this.element = canvas;
...
//#endregion
//#region node_modules/@tsparticles/engine/browser/Enums/InteractivityDetect.js
...
var EventListeners = class {
	constructor(container) {
		...
		this._handleThemeChange = (e) => {
			const mediaEvent = e, container = this.container, options = container.options, defaultThemes = options.defaultThemes, themeName = mediaEvent.matches ? defaultThemes.dark : defaultThemes.light;
			if (options.themes.find((theme) => theme.name === themeName)?.default.auto) container.loadTheme(themeName);
		};
		...
		this._manageInteractivityListeners = (mouseLeaveTmpEvent, add) => {
			...
			if (!options.interactivity.events.onClick.enable) manageListener(interactivityEl, touchEndEvent, handlers.touchEnd, add);
			else {
		...
		this._manageListeners = (add) => {
			const handlers = this._handlers, container = this.container, detectType = container.actualOptions.interactivity.detectsOn, canvasEl = container.canvas.element;
			let mouseLeaveTmpEvent = mouseLeaveEvent;
			if (detectType === InteractivityDetect.window) {
				container.interactivity.element = window;
				mouseLeaveTmpEvent = mouseOutEvent;
			} else if (detectType === InteractivityDetect.parent && canvasEl) container.interactivity.element = canvasEl.parentElement ?? canvasEl.parentNode;
			else container.interactivity.element = canvasEl;
			this._manageMediaMatch(add);
			this._manageResize(add);
			this._manageInteractivityListeners(mouseLeaveTmpEvent, add);
			if (document) manageListener(document, visibilityChangeEvent, handlers.visibilityChange, add, false);
		};
		this._manageMediaMatch = (add) => {
			...
			if (mediaMatch.addListener === void 0) return;
			if (add) mediaMatch.addListener(handlers.oldThemeChange);
			else mediaMatch.removeListener(handlers.oldThemeChange);
		};
		this._manageResize = (add) => {
			const handlers = this._handlers, container = this.container;
			if (!container.actualOptions.interactivity.events.resize) return;
...
//#endregion
//#region node_modules/@tsparticles/engine/browser/Options/Classes/BackgroundMask/BackgroundMaskCover.js
...
//#endregion
//#region node_modules/@tsparticles/engine/browser/Options/Classes/BackgroundMask/BackgroundMask.js
var BackgroundMask = class {
	...
	load(data) {
		if (isNull(data)) return;
		if (data.composite !== void 0) this.composite = data.composite;
...
//#endregion
//#region node_modules/@tsparticles/engine/browser/Options/Classes/Interactivity/Events/ClickEvent.js
...
//#endregion
//#region node_modules/@tsparticles/engine/browser/Enums/Types/DivType.js
var DivType;
(function(DivType) {
...
//#endregion
//#region node_modules/@tsparticles/engine/browser/Options/Classes/Interactivity/Events/Parallax.js
var Parallax = class {
	constructor() {
		this.enable = false;
		this.force = 2;
	...
	load(data) {
		if (isNull(data)) return;
		if (data.enable !== void 0) this.enable = data.enable;
		if (data.force !== void 0) this.force = data.force;
		if (data.smooth !== void 0) this.smooth = data.smooth;
	}
};
//#endregion
//#region node_modules/@tsparticles/engine/browser/Options/Classes/Interactivity/Events/HoverEvent.js
...
//#endregion
//#region node_modules/@tsparticles/engine/browser/Options/Classes/Interactivity/Events/ResizeEvent.js
...
//#endregion
//#region node_modules/@tsparticles/engine/browser/Options/Classes/Interactivity/Events/Events.js
var Events = class {
	constructor() {
...
//#endregion
//#region node_modules/@tsparticles/engine/browser/Options/Classes/Interactivity/Modes/Modes.js
var Modes = class {
...
//#endregion
//#region node_modules/@tsparticles/engine/browser/Options/Classes/Interactivity/Interactivity.js
...
//#endregion
//#region node_modules/@tsparticles/engine/browser/Options/Classes/ManualParticle.js
var ManualParticle = class {
	load(data) {
		...
		if (data.options) this.options = deepExtend({}, data.options);
	}
};
//#endregion
//#region node_modules/@tsparticles/engine/browser/Enums/Modes/ResponsiveMode.js
var ResponsiveMode;
...
var Responsive = class {
	...
	load(data) {
		if (isNull(data)) return;
		if (!isNull(data.maxWidth)) this.maxWidth = data.maxWidth;
		if (!isNull(data.mode)) if (data.mode === ResponsiveMode.screen) this.mode = ResponsiveMode.screen;
		else this.mode = ResponsiveMode.canvas;
		if (!isNull(data.options)) this.options = deepExtend({}, data.options);
	}
};
//#endregion
//#region node_modules/@tsparticles/engine/browser/Enums/Modes/ThemeMode.js
var ThemeMode;
(function(ThemeMode) {
	ThemeMode["any"] = "any";
	ThemeMode["dark"] = "dark";
...
var ThemeDefault = class {
	...
	load(data) {
		if (isNull(data)) return;
		if (data.auto !== void 0) this.auto = data.auto;
		if (data.mode !== void 0) this.mode = data.mode;
		if (data.value !== void 0) this.value = data.value;
	}
};
//#endregion
//#region node_modules/@tsparticles/engine/browser/Options/Classes/Theme/Theme.js
var Theme = class {
	constructor() {
		this.name = "";
...
var AnimatableColor = class AnimatableColor extends OptionsColor {
	...
	load(data) {
		...
		if (colorAnimation !== void 0) if (colorAnimation.enable !== void 0) this.animation.h.load(colorAnimation);
		else this.animation.load(data.animation);
	}
};
//#endregion
//#region node_modules/@tsparticles/engine/browser/Enums/Modes/CollisionMode.js
var CollisionMode;
...
//#endregion
//#region node_modules/@tsparticles/engine/browser/Options/Classes/Particles/Collisions/CollisionsOverlap.js
var CollisionsOverlap = class {
	...
	load(data) {
		if (isNull(data)) return;
		if (data.enable !== void 0) this.enable = data.enable;
		if (data.retries !== void 0) this.retries = data.retries;
	}
};
//#endregion
...
//#endregion
//#region node_modules/@tsparticles/engine/browser/Options/Classes/Particles/Collisions/Collisions.js
...
//#endregion
//#region node_modules/@tsparticles/engine/browser/Options/Classes/Particles/Move/MoveAttract.js
...
//#endregion
//#region node_modules/@tsparticles/engine/browser/Options/Classes/Particles/Move/MoveTrailFill.js
...
//#endregion
//#region node_modules/@tsparticles/engine/browser/Options/Classes/Particles/Move/MoveTrail.js
var MoveTrail = class {
	constructor() {
		this.enable = false;
		this.length = 10;
	...
	load(data) {
		if (isNull(data)) return;
		if (data.enable !== void 0) this.enable = data.enable;
		if (data.fill !== void 0) this.fill.load(data.fill);
		if (data.length !== void 0) this.length = data.length;
	}
};
//#endregion
...
//#endregion
//#region node_modules/@tsparticles/engine/browser/Options/Classes/Particles/Opacity/OpacityAnimation.js
...
//#endregion
//#region node_modules/@tsparticles/engine/browser/Options/Classes/Particles/Opacity/Opacity.js
...
//#endregion
//#region node_modules/@tsparticles/engine/browser/Options/Classes/Particles/Shadow.js
var Shadow = class {
	constructor() {
		this.blur = 0;
		this.color = new OptionsColor();
	...
	load(data) {
		...
		if (data.enable !== void 0) this.enable = data.enable;
		if (data.offset === void 0) return;
		if (data.offset.x !== void 0) this.offset.x = data.offset.x;
		if (data.offset.y !== void 0) this.offset.y = data.offset.y;
	}
};
//#endregion
...
//#endregion
//#region node_modules/@tsparticles/engine/browser/Options/Classes/Particles/Size/SizeAnimation.js
...
//#endregion
//#region node_modules/@tsparticles/engine/browser/Options/Classes/Particles/Size/Size.js
...
var ParticlesOptions = class {
	...
	load(data) {
		...
		if (data.interactivity !== void 0) this.interactivity = deepExtend({}, data.interactivity);
		const strokeToLoad = data.stroke;
		if (strokeToLoad) this.stroke = executeOnSingleOrMultiple(strokeToLoad, (t) => {
...
var Options = class {
	...
	load(data) {
		if (isNull(data)) return;
		if (data.preset !== void 0) executeOnSingleOrMultiple(data.preset, (preset) => this._importPreset(preset));
		...
		if (data.responsive !== void 0) for (const responsive of data.responsive) {
			const optResponsive = new Responsive();
			optResponsive.load(responsive);
			this.responsive.push(optResponsive);
		}
		this.responsive.sort((a, b) => a.maxWidth - b.maxWidth);
		if (data.themes !== void 0) for (const theme of data.themes) {
	...
	setTheme(name) {
		if (name) {
			...
			const clientDarkMode = safeMatchMedia("(prefers-color-scheme: dark)")?.matches, defaultTheme = this._findDefaultTheme(clientDarkMode ? ThemeMode.dark : ThemeMode.light);
			if (defaultTheme) this.load(defaultTheme.options);
		}
	}
};
//#endregion
//#region node_modules/@tsparticles/engine/browser/Enums/Types/InteractorType.js
var InteractorType;
...
var Particle = class {
	constructor(engine, container) {
		...
		this._calcPosition = (container, position, zIndex, tryCount = 0) => {
			...
			if (this._checkOverlap(pos, tryCount)) return this._calcPosition(container, void 0, zIndex, tryCount + 1);
			return pos;
		};
		this._calculateVelocity = () => {
			const res = getParticleBaseVelocity(this.direction).copy(), moveOptions = this.options.move;
			...
			if (!moveOptions.straight) res.angle += randomInRange(setRangeValue(range.left, range.right));
		...
		this._checkOverlap = (pos, tryCount = 0) => {
			...
			if (retries >= 0 && tryCount > retries) throw new Error(`${errorPrefix} particle is overlapping and can't be placed`);
			return !!this.container.particles.find((particle) => getDistance(pos, particle.position) < radius + particle.getRadius());
		};
		this._getRollColor = (color) => {
			if (!color || !this.roll || !this.backColor && !this.roll.alter) return color;
			const backFactor = this.roll.horizontal && this.roll.vertical ? 2 : 1, backSum = this.roll.horizontal ? Math.PI * half : 0;
			if (!(Math.floor(((this.roll.angle ?? 0) + backSum) / (Math.PI / backFactor)) % 2)) return color;
	...
	destroy(override) {
		...
		container.shapeDrawers.get(this.shape)?.particleDestroy?.(this);
...
//#endregion
//#region node_modules/@tsparticles/engine/browser/Core/Utils/Point.js
var Point = class {
...
//#endregion
//#region node_modules/@tsparticles/engine/browser/Types/RangeType.js
var RangeType;
(function(RangeType) {
...
var Circle = class Circle extends BaseRange {
	...
	intersects(range) {
		...
		if (range instanceof Circle || range.type === RangeType.circle) return r + range.radius > Math.sqrt(distPos.x ** 2 + distPos.y ** 2);
		else if (range instanceof Rectangle || range.type === RangeType.rectangle) {
			const { width, height } = range.size;
			return Math.pow(distPos.x - width, 2) + Math.pow(distPos.y - height, 2) <= r ** 2 || distPos.x <= r + width && distPos.y <= r + height || distPos.x <= width || distPos.y <= height;
		}
		return false;
	}
};
...
//#endregion
//#region node_modules/@tsparticles/engine/browser/Core/Utils/QuadTree.js
var QuadTree = class QuadTree {
	...
	query(range, check) {
		...
		for (const p of this._points) {
			if (!range.contains(p.position) && getDistance(range.position, p.position) > p.particle.getRadius() && (!check || check(p.particle))) continue;
			res.push(p.particle);
		}
...
//#endregion
//#region node_modules/@tsparticles/engine/browser/Core/Particles.js
...
var Particles = class {
	constructor(engine, container) {
		...
		this._applyDensity = (options, manualCount, group, groupOptions) => {
			...
			if (!options.number.density?.enable) {
				if (group === void 0) this._limit = numberOptions.limit.value;
				else if (groupOptions?.number.limit?.value ?? numberOptions.limit.value) this._groupLimits.set(group, groupOptions?.number.limit?.value ?? numberOptions.limit.value);
				return;
			}
			...
			if (group === void 0) this._limit = numberOptions.limit.value * densityFactor;
		...
		this._pushParticle = (position, overrideOptions, group, initializer) => {
			try {
				...
				if (!particle) particle = new Particle(this._engine, this._container);
		...
		const canvasSize = container.canvas.size;
		this.quadTree = new QuadTree(qTreeRectangle(canvasSize), 4);
		this.movers = [];
		this.updaters = [];
	}
	get count() {
	...
	addManualParticles() {
		const container = this._container;
		container.actualOptions.manualParticles.forEach((p) => this.addParticle(p.position ? getPosition(p.position, container.canvas.size) : void 0, p.options));
	}
	...
	update(delta) {
		...
		this.quadTree = new QuadTree(qTreeRectangle(container.canvas.size), 4);
		...
		if (particlesToDelete.size) {
			...
			for (const particle of particlesToDelete) this._engine.dispatchEvent(EventType.particleRemoved, {
				container: this._container,
				data: { particle }
			});
...
var Retina = class {
	...
	init() {
		const container = this.container, options = container.actualOptions;
		this.pixelRatio = !options.detectRetina || isSsr() ? 1 : devicePixelRatio;
		this.reduceFactor = 1;
...
function initDelta(value, fpsLimit = 60, smooth = false) {
	return {
		value,
		factor: smooth ? 60 / fpsLimit : 60 * value / millisecondsToSeconds
	};
}
...
var Container = class {
	...
	addClickHandler(callback) {
		...
		const clickOrTouchHandler = (e, pos, radius) => {
			...
			callback(e, this.particles.quadTree.queryCircle(posRetina, radius * pxRatio));
		}, clickHandler = (e) => {
			if (!guardCheck(this)) return;
			const mouseEvent = e;
			clickOrTouchHandler(e, {
				x: mouseEvent.offsetX || mouseEvent.clientX,
				y: mouseEvent.offsetY || mouseEvent.clientY
			}, 1);
		}, touchStartHandler = () => {
			...
			if (touched && !touchMoved) {
				...
				const element = this.canvas.element, canvasRect = element ? element.getBoundingClientRect() : void 0;
				clickOrTouchHandler(e, {
...
var EventDispatcher = class {
	...
	removeAllEventListeners(type) {
		if (!type) this._listeners = /* @__PURE__ */ new Map();
...
var getCanvasFromContainer = (domContainer) => {
	...
	if (domContainer instanceof HTMLCanvasElement || domContainer.tagName.toLowerCase() === "canvas") {
		...
		if (!canvasEl.dataset["generated"]) canvasEl.dataset[generatedAttribute] = generatedFalse;
	} else {
...
var Engine = class {
	...
	addConfig(config) {
		...
		this._eventDispatcher.dispatchEvent(EventType.configAdded, { data: {
			name: key,
			config
		} });
	}
	...
	async load(params) {
		const id = params.id ?? params.element?.id ?? `tsparticles${Math.floor(getRandom() * 1e4)}`, { index, url } = params, currentOptions = itemFromSingleOrMultiple(url ? await getDataFromUrl({
			fallback: params.options,
			url,
			index
		}) : params.options, index), { items } = this, oldIndex = items.findIndex((v) => v.id.description === id), newItem = new Container(this, id, currentOptions);
		if (oldIndex >= 0) {
			const old = this.item(oldIndex), deleteCount = old ? 1 : 0;
			if (old && !old.destroyed) old.destroy(false);
			items.splice(oldIndex, deleteCount, newItem);
		} else items.push(newItem);
		const canvasEl = getCanvasFromContainer(getDomContainer(id, params.element));
	...
	setOnClickHandler(callback) {
		const { items } = this;
		if (!items.length) throw new Error(`${errorPrefix} can only set click handlers after calling tsParticles.load()`);
...
if (!isSsr()) window.tsParticles = tsParticles;
//#endregion
export { randomInRange as $, executeOnSingleOrMultiple as A, rectBounce as B, updateColor as C, deepExtend as D, circleBounceDataFromParticle as E, isPointInside as F, clamp as G, OutModeDirection as H, isSsr as I, getDistances as J, degToRad as K, itemFromArray as L, initParticleNumericAnimationValue as M, isDivModeEnabled as N, divMode as O, isInArray as P, parseAlpha as Q, itemFromSingleOrMultiple as R, rgbToHsl as S, circleBounce as T, DestroyType as U, updateAnimation as V, AnimationStatus as W, getRangeMax as X, getRandom as Y, getRangeValue as Z, getStyleFromRgb as _, InteractorType as a, errorPrefix as at, rangeColorToHsl as b, CollisionMode as c, mouseMoveEvent as ct, drawLine as d, setRangeValue as et, colorMix as f, getStyleFromHsl as g, getLinkRandomColor as h, ParticleOutType as i, isObject as it, getLogger as j, divModeExecute as k, DivType as l, getLinkColor as m, Circle as n, isArray as nt, OutMode as o, millisecondsToSeconds as ot, getHslAnimationFromHsl as p, getDistance as q, Rectangle as r, isNull as rt, ValueWithRandom as s, mouseLeaveEvent as st, tsParticles as t, Vector as tt, OptionsColor as u, hslToRgb as v, calculateBounds as w, rangeColorToRgb as x, hslaToRgba as y, loadFont as z };

//# sourceMappingURL=browser-DRi8p1LK.js.map